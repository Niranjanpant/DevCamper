...
exports.createCourse = asyncHandler(async (req, res, next) => {
  ...
  //make sure the user associated with bootcamp can create a course
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `user ${req.user.id} is not authorize to add a course to bootcammp ${bootcamp._id}`,
        401
      )
    );
  }
...
exports.updateCourse = asyncHandler(async (req, res, next) => {
  ...
  //make sure the user is a course owner
  if (course.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `user ${req.user.id} is not authorize to updatea course ${course._id}`,
        401
      )
    );
  }
